...
* An `underlying system` is the thing we are trying to understand better.

* A `model` is an abstract representation of an underlying system.  The purpose of a model is to approximating the underlying system behaviours,
...
* Given a system and scenario, a (simulation, configuration) pair can be constructed and executed by a simulator to generate runs.
...
Fitting is the particular cases where the constraint is the approximation error between a simulation outputs and the system observations.
